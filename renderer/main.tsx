...
if (!window.assistant) {
  window.assistant = {
    ...
    focus: {
      create: async () => ({ success: true, focusId: 'mock-focus' }),
      updateMetadata: async () => ({ success: true }),
      checkIn: async () => ({ success: true, checkInId: 'mock-checkin' }),
      get: async () => null,
      list: async () => [],
      alerts: async () => [],
      checkins: async () => [],
      stats: async () => ({
        totalAreas: 0,
        activeAreas: 0,
        modeCounts: { deep: 0, pulse: 0, scan: 0, dormant: 0 },
        healthCounts: { aligned: 0, drifting: 0, neglected: 0, cooling: 0 },
        alertCount: 0,
        totalCheckIns: 0,
        checkInsLast7Days: 0,
        averageWeight: 0,
